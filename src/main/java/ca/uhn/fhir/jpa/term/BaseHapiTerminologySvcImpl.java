...
public abstract class BaseHapiTerminologySvcImpl implements IHapiTerminologySvc, ApplicationContextAware {
    ...
    // Adding this as a temporary speed up ... TODO - come up with something better - what happens when ValueSet is updated?
    private Map<String, ValueSet> expansionCache = new HashMap<>();
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public ValueSet expandValueSet(ValueSet theValueSetToExpand) {
        if (expansionCache.containsKey(theValueSetToExpand.getIdElement().getIdPart())) {
            return expansionCache.get(theValueSetToExpand.getIdElement().getIdPart());
        }
        ...
        expansionCache.put(theValueSetToExpand.getIdElement().getIdPart(), valueSet);
        return valueSet;
    }

    protected List<VersionIndependentConcept> expandValueSetAndReturnVersionIndependentConcepts(ValueSet theValueSetToExpandR4) {
        ValueSet.ValueSetExpansionComponent expandedR4 = expandValueSet(theValueSetToExpandR4).getExpansion();

        ArrayList<VersionIndependentConcept> retVal = new ArrayList<>();
        for (ValueSet.ValueSetExpansionContainsComponent nextContains : expandedR4.getContains()) {
            retVal.add(
