...
public class FHIRPlanDefinitionResourceProvider extends PlanDefinitionResourceProvider {
    ...
    public FHIRPlanDefinitionResourceProvider(ServerManager manager) {
        this.manager = manager;
        executionProvider = new CqlExecutionProvider(manager);
    }
    ...
    private void resolveDefinition(Session session, PlanDefinition.PlanDefinitionActionComponent action) {
        if (action.hasDefinition()) {
            ...
            if (definition.getReference().startsWith(session.getPlanDefinition().fhirType())) {
                ...
                FHIRActivityDefinitionResourceProvider activitydefinitionProvider = new FHIRActivityDefinitionResourceProvider(manager);
                Resource result;
                try {
        ...
        for (PlanDefinition.PlanDefinitionActionDynamicValueComponent dynamicValue: action.getDynamicValue())
        {
            ...
            if (dynamicValue.hasExpression()) {
                ...
                if (dynamicValue.hasPath() && dynamicValue.getPath().equals("$this"))
                {
                    ...
                    manager.getDataProvider().setValue(session.getCarePlan(), dynamicValue.getPath(), result);
                }
            }
        }
    }
                ...
                if (conditionsMet) {
                    ...
                    if (action.hasDefinition()) {
                        if (action.getDefinition().getReferenceElement().getResourceType().equals("ActivityDefinition")) {
                            ...
                            // TODO - fix this
                            FHIRActivityDefinitionResourceProvider activitydefinitionProvider = (FHIRActivityDefinitionResourceProvider) manager.getDataProvider().resolveResourceProvider("ActivityDefinition");
                ...
                if (typeCode.getCode().equals("eca-rule")) {
                    if (planDefinition.hasLibrary()) {
                        for (Reference reference : planDefinition.getLibrary()) {
                            org.cqframework.cql.elm.execution.Library library = manager.getLibraryLoader().load(
                            ...
                            discoveryDataProvider.setTerminologyProvider(manager.getDataProvider().getTerminologyProvider());
